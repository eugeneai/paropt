...
CCCCCCC%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	REAL*8 Y(K,L)
CCC ПЕЧАТЬ МАТРИЦЫ С 8-Ю СТОЛБЦАМИ
106   FORMAT(8(D16.9,' '))
      DO 331 I=1,K
...
106    FORMAT(8(D16.9,' '))
	  ...
	  write(53,'(A20,i2)')'H_U1 итерация', iter
...
C	 ТРАНСПОНИРУЕМ МАТРИЦУ S - stt
       call transpon(s,n1,m,stt)
	 WRITE (50,'(A27,i2)')'МАТРИЦА S итерация',iter
	 call PECHAT11(stt,m,n1)
	 WRITE (50,'(A20,i2)')'МАТРИЦА V итерация',iter
	 WRITE (50, 106) v(1),v(2),v(3),
     *v(4),v(5),v(6),v(7),v(8)
CC КОНЕЦ ПЕЧАТИ ТРАЕКТОРИИ
CC ПЕЧАТЬ УПРАВЛЕНИЙ
	 write(50,'(A20)')'УПРАВЛЕНИЯ'
        call transpon(u1,n1,n,u1tr)
        call transpon(u2,n1,n,u2tr)
	  call transpon(v1,n1,n,v1tr)
	 write(50,'(A20)')'U1'
