...
CCCCCC ÏĞÎÖÅÄÓĞÀ ÍÀÕÎÆÄÅÍÈß ÒĞÀÅÊÒÎĞÈÈ ÏËÎÙÀÄÅÉ ËÅÑÀ ÍÀ {t0,t0+1,...,t1}
...
CC%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
CCC ÇÀÄÀŞÒÑß ÍÀ×ÀËÜÍÛÅ ÄÀÍÍÛÅ
CC     ÍÀ×ÀËÜÍÎÅ ÑÎÑÒÎßÍÈÅ v_0 - ìîùíîñòü Áèğşñèíñêîãî ËÏÕ (ì3/ãîä)
	v(1)=2465170.D0
CC     ÍÀ×ÀËÜÍÎÅ ÑÎÑÒÎßÍÈÅ s_0 - ïëîùàäè ğàçíûõ òèïîâ è âèäîâ ëåñà (ãà)
	s(1,1)=328.97D0
	s(1,2)=0.D0
	s(1,3)=779.17D0
	s(1,4)=1753.03D0
...
C ÍÀ×ÀËÜÍÎÅ ÂĞÅÌß
	t=tn
CCCCC ÖÈÊË ÏĞßÌÎÉ ÏÎ ÂĞÅÌÅÍÈ
	do 1 nt=1,nn
C	  ÂÛ×ÈÑËÅÍÈÅ ÒĞÀÅÊÒÎĞÈÈ Â ÃÎÄ (T)
	  vt=v(nt)		! VT - ÌÎÙÍÎÑÒÜ ÏĞÅÄÏĞÈßÒÈß Â ÃÎÄ (T)
c	  ÏĞÎÂÅĞßÅÌ S ÍÀ ÍÅÎÒĞÈÖÀÒÅËÜÍÎÑÒÜ
	  do 3 i=1,m
	    st(i,1)=s(nt,i)	! ST - ÏËÎÙÀÄÜ ÒÈÏÎÂ ËÅÑÀ Â ÃÎÄ (T)
3	 continue
c	  ÊÎÍÅÖ ÏĞÎÂÅĞÊÈ

C	  ÖÈÊË ÏÎ ÓÏĞÀÂËÅÍÈŞ
	  u3t=u3(nt)	! ğàñøèğåíèå ìîùíîñòè ïğåäïğèÿòèÿ Â ÃÎÄ (T)
	  do 6 i=1,n
	  u2t(i,1)=u2(nt,i)	! ËÅÑÎÏÎÑÀÄÊÈ Â ÃÎÄ (T)
6	  u1t(i,1)=u1(nt,i)	! ÂÛĞÓÁÊÈ Â ÃÎÄ (T)
C	  ÊÎÍÅÖ ÖÈÊËÀ ÏÎ ÓÏĞÀÂËÅÍÈŞ	

CCCCC   ÏĞÀÂÀß ×ÀÑÒÜ Â ÃÎÄ (T)
	  call fp(n,m,vt,st,h1,u3t,A,B1,u1t,B2,u2t,d,l,f1,f)
	  do 7 i=1,m

7	    s(nt+1,i)=f(i,1)
	  v(nt+1)=f1
	  t=t+h1   ! ÓÂÅËÈ×ÈÂÀÅÌ ÂĞÅÌß ÍÀ ÎÄÈÍ ÃÎÄ
1	continue


CCC ÏÅ×ÀÒÜ ÒĞÀÅÊÒÎĞÈÉ
CCCCC ÏÎÄÑ×ÅÒ ÔÓÍÊÖÈÎÍÀËÀ
...
CCCCCC ÏĞÎÖÅÄÓĞÀ ÏÎÑ×ÅÒÀ ÔÓÍÊÖÈÎÍÀËÀ
CCCCCC fln - ôóíêöèîíàë	îñíîâíîé
C      fl - øòğàôíîé
	integer n2
...
C ÏÎÄÑ×ÅÒ ÏÎÄ ÇÍÀÊÎÌ ÑÓÌÌÛ
	do 3 nt=1,nn
c ôîğìèğóåì òğàåêòîğèş
	 vt=v(nt)
	 do 30 i=1,m
30	 st(i,1)=s(nt,i)
C çàäàåì óïğàâëåíèå
	 u3t=u3(nt)
       do 6 i=1,n
        u2t(i,1)=u2(nt,i)	! ëåñîïîñàäêè â ìîìåíò t
6	  v1t(i)=v1(nt,i)	! âûğóáêè â 3-ì òèïå ëåñà

	   call f0p(n,vt,h1,u3t,u2t,v1t,gamma,delta,b,c,f0)
	   call f0ss(n,m,h1,g,n2,vt,st,v1t,sf0,f0s)
c	  print *,'f0s',f0s
C ÇÍÀ×ÅÍÈÅ ÔÓÍÊÖÈÎÍÀËÀ	
	 fln=fln+f0 ! FLN -ÎÁÛ×ÍÛÉ ÔÓÍÊÖÈÎÍÀË
	 fl=fl+f0s ! FL - ØÒĞÀÔÍÎÉ ÔÓÍÊÖÈÎÍÀË
3	 t=t+h1
c s0
c	fln=s0*fln+(1.D0-s0)*fl
	p=fl*rk/2.D0
	fln=fln+p
103	format(/,20x,'vsego fln=',D16.9,'èç íèõ  p=',D16.9)
	write(77,103) fln,p
	razn=fln-p
	WRITE(77,'(A20,D16.9)')'ĞÀÇÍÎÑÒÜ=',razn
...
C100   format(2x,'íåâÿçêà',4D10.4)
C101   format(2x,'íåâÿçêè=')
C102	format(2x,'s0f',4D10.4)
C104   format(2x,'øòğàô=')
